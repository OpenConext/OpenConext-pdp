 ...
 */
  ...
  const focusNode = _dereq_("./focusNode");
 ...
 */
  ...
  const EventPropagators = _dereq_("./EventPropagators");
  const ExecutionEnvironment = _dereq_("./ExecutionEnvironment");
  const SyntheticInputEvent = _dereq_("./SyntheticInputEvent");
 ...
 */
  ...
  const CSSProperty = _dereq_("./CSSProperty");

  const dangerousStyleValue = _dereq_("./dangerousStyleValue");
  const hyphenateStyleName = _dereq_("./hyphenateStyleName");
  const memoizeStringOnly = _dereq_("./memoizeStringOnly");
    ...
    setValueForStyles: function(node, styles) {
      ...
      for (const styleName in styles) {
        ...
        if (styleValue) {
          ...
          const expansion = CSSProperty.shorthandPropertyExpansions[styleName];
          if (expansion) {
 ...
 */
  ...
  const EventPropagators = _dereq_("./EventPropagators");
  const ExecutionEnvironment = _dereq_("./ExecutionEnvironment");
  const ReactUpdates = _dereq_("./ReactUpdates");
  const SyntheticEvent = _dereq_("./SyntheticEvent");

  const isEventSupported = _dereq_("./isEventSupported");
  const isTextInputElement = _dereq_("./isTextInputElement");
  const keyOf = _dereq_("./keyOf");
 ...
 */
   ...
   */
    extractEvents: function(
      ...
      let getTargetIDFunc, handleEventFunc;
      ...
      if (getTargetIDFunc) {
        const targetID = getTargetIDFunc(
        topLevelType,
        topLevelTarget,
        topLevelTargetID
      );
 ...
 */
  ...
  const EventPropagators = _dereq_("./EventPropagators");
  const ExecutionEnvironment = _dereq_("./ExecutionEnvironment");
  const ReactInputSelection = _dereq_("./ReactInputSelection");
  const SyntheticCompositionEvent = _dereq_("./SyntheticCompositionEvent");

  const getTextContentAccessor = _dereq_("./getTextContentAccessor");
  const keyOf = _dereq_("./keyOf");
 ...
 */
  FallbackCompositionState.prototype.getData = function() {
    ...
    const prefixLength = this.startSelection.start;
    const suffixLength = this.startValue.length - this.startSelection.end;
 ...
 */
  ...
  const Danger = _dereq_("./Danger");
  const ReactMultiChildUpdateTypes = _dereq_("./ReactMultiChildUpdateTypes");
 ...
 * @private
 */
  const textContentAccessor = getTextContentAccessor();
 ...
 */
   ...
   */
    ...
    // List of children that will be moved or removed.
      ...
      for (let i = 0; update = updates[i]; i++) {
        if (update.type === ReactMultiChildUpdateTypes.MOVE_EXISTING ||
          update.type === ReactMultiChildUpdateTypes.REMOVE_NODE) {
          const updatedIndex = update.fromIndex;
          const updatedChild = update.parentNode.childNodes[updatedIndex];
      ...
      const renderedMarkup = Danger.dangerouslyRenderMarkup(markupList);
    ...
    // Remove updated children first so that `toIndex` is consistent.
      ...
      for (let k = 0; update = updates[k]; k++) {
        switch (update.type) {
 ...
 */
   ...
   */
    getDefaultValueForProperty: function(nodeName, prop) {
      let nodeDefaults = defaultValueCache[nodeName];
      let testElement;
      if (!nodeDefaults) {
 ...
 */
  ...
  const DOMProperty = _dereq_("./DOMProperty");

  const escapeTextForBrowser = _dereq_("./escapeTextForBrowser");
  const memoizeStringOnly = _dereq_("./memoizeStringOnly");
  ...
  const processAttributeNameAndPrefix = memoizeStringOnly(name => {
    ...
    deleteValueForProperty: function(node, name) {
      if (DOMProperty.isStandardName.hasOwnProperty(name) &&
        ...
        if (mutationMethod) {
          ...
          const propName = DOMProperty.getPropertyName[name];
          const defaultValue = DOMProperty.getDefaultValueForProperty(
          node.nodeName,
          propName
        );
...
/*jslint evil: true, sub: true */
  ...
  const createNodesFromMarkup = _dereq_("./createNodesFromMarkup");
  const emptyFunction = _dereq_("./emptyFunction");
  const getMarkupWrap = _dereq_("./getMarkupWrap");
  const invariant = _dereq_("./invariant");
 ...
 */
  ...
  const EventConstants = _dereq_("./EventConstants");
  const EventPropagators = _dereq_("./EventPropagators");
  const SyntheticMouseEvent = _dereq_("./SyntheticMouseEvent");

  const ReactMount = _dereq_("./ReactMount");
  const keyOf = _dereq_("./keyOf");

  const topLevelTypes = EventConstants.topLevelTypes;
  const getFirstReactDOM = ReactMount.getFirstReactDOM;
  ...
  const extractedEvents = [null, null];
   ...
   */
    extractEvents: function(
      ...
      // TODO: Figure out why `ownerDocument` is sometimes undefined in IE8.
        const doc = topLevelTarget.ownerDocument;
        if (doc) {
      ...
      const fromID = from ? ReactMount.getID(from) : "";
      const toID = to ? ReactMount.getID(to) : "";
 ...
 */
  function accumulateDirectionalDispatches(domID, upwards, event) {
    ...
    const listener = listenerAtPhase(domID, event, phase);
    if (listener) {
 ...
 */
  function accumulateDispatches(id, ignoredDirection, event) {
    if (event && event.dispatchConfig.registrationName) {
      ...
      const listener = getListener(id, registrationName);
      if (listener) {
        event._dispatchListeners = accumulate(event._dispatchListeners, listener);
 ...
 */
  ...
  const ReactLink = _dereq_("./ReactLink");
  const ReactStateSetters = _dereq_("./ReactStateSetters");
 ...
 */
  ...
  const ReactPropTypes = _dereq_("./ReactPropTypes");
 ...
 */
  ...
  const LocalEventTrapMixin = {
    trapBubbledEvent:function(topLevelType, handlerBaseName) {
      ("production" !== "development" ? invariant(this.isMounted(), "Must be mounted to trap events") : invariant(this.isMounted()));
      const listener = ReactBrowserEventEmitter.trapBubbledEvent(
      topLevelType,
      handlerBaseName,
      this.getDOMNode()
    );
 ...
 */
  const MobileSafariClickEventPlugin = {
 ...
 */
  ...
  const DOMPropertyOperations = _dereq_("./DOMPropertyOperations");
  const EventPluginUtils = _dereq_("./EventPluginUtils");
  const ReactChildren = _dereq_("./ReactChildren");
  const ReactComponent = _dereq_("./ReactComponent");
  const ReactCompositeComponent = _dereq_("./ReactCompositeComponent");
  const ReactContext = _dereq_("./ReactContext");
  const ReactCurrentOwner = _dereq_("./ReactCurrentOwner");
  const ReactDescriptor = _dereq_("./ReactDescriptor");
  const ReactDOM = _dereq_("./ReactDOM");
  const ReactDOMComponent = _dereq_("./ReactDOMComponent");
  const ReactDefaultInjection = _dereq_("./ReactDefaultInjection");
  const ReactInstanceHandles = _dereq_("./ReactInstanceHandles");
  const ReactMount = _dereq_("./ReactMount");
  const ReactMultiChild = _dereq_("./ReactMultiChild");
  const ReactPerf = _dereq_("./ReactPerf");
  const ReactPropTypes = _dereq_("./ReactPropTypes");
  const ReactServerRendering = _dereq_("./ReactServerRendering");
  const ReactTextComponent = _dereq_("./ReactTextComponent");

  const onlyChild = _dereq_("./onlyChild");
  ...
  if ("production" !== "development") {
    ...
    if (ExecutionEnvironment.canUseDOM &&
      ...
      for (const i in expectedFeatures) {
 ...
 */
  ...
  const ReactEmptyComponent = _dereq_("./ReactEmptyComponent");
  const ReactMount = _dereq_("./ReactMount");

  const invariant = _dereq_("./invariant");

  const ReactBrowserComponentMixin = {
  /**
 ...
 * @internal
 */
  const ReactBrowserEventEmitter = merge(ReactEventEmitterMixin, {
 ...
 */
  ...
  const ReactTransitionGroup = _dereq_("./ReactTransitionGroup");
  const ReactCSSTransitionGroupChild = _dereq_("./ReactCSSTransitionGroupChild");
 ...
 */
  ...
  const CSSCore = _dereq_("./CSSCore");
  const ReactTransitionEvents = _dereq_("./ReactTransitionEvents");
 ...
 */
  ...
  const traverseAllChildren = _dereq_("./traverseAllChildren");
  const warning = _dereq_("./warning");

  const twoArgumentPooler = PooledClass.twoArgumentPooler;
  const threeArgumentPooler = PooledClass.threeArgumentPooler;
 ...
 */
  ...
  function forEachSingleChild(traverseContext, child, name, i) {
    const forEachBookKeeping = traverseContext;
 ...
 */
  ...
  function mapSingleChildIntoContext(traverseContext, child, name, i) {
    const mapBookKeeping = traverseContext;
    const mapResult = mapBookKeeping.mapResult;

    const keyUnique = !mapResult.hasOwnProperty(name);
    ("production" !== "development" ? warning(
    keyUnique,
 ...
 */
  function mapChildren(children, func, context) {
    ...
    const mapResult = {};
    const traverseContext = MapBookKeeping.getPooled(mapResult, func, context);
 ...
 */
  ...
  const ReactDescriptor = _dereq_("./ReactDescriptor");
  const ReactOwner = _dereq_("./ReactOwner");
  const ReactUpdates = _dereq_("./ReactUpdates");
 ...
 * @private
 */
  let unmountIDFromEnvironment = null;
 ...
 */
   ...
   */
     ...
     */
      ...
      // descriptor props.
        const descriptor = this._pendingDescriptor || this._descriptor;
        this.replaceProps(
     ...
     */
      mountComponent: function(rootID, transaction, mountDepth) {
        ...
        if (props.ref != null) {
          const owner = this._descriptor._owner;
     ...
     */
      performUpdateIfNecessary: function(transaction) {
        ...
        const prevDescriptor = this._descriptor;
        const nextDescriptor = this._pendingDescriptor;
     ...
     */
      updateComponent: function(transaction, prevDescriptor) {
        const nextDescriptor = this._descriptor;
     ...
     */
      _mountComponentIntoNode: function(
        ...
        transaction,
        shouldReuseMarkup) {
        const markup = this.mountComponent(rootID, transaction, 0);
...
/*jslint evil: true */
  ...
  const ReactDOMIDOperations = _dereq_("./ReactDOMIDOperations");
  const ReactMarkupChecksum = _dereq_("./ReactMarkupChecksum");
  const ReactMount = _dereq_("./ReactMount");
  const ReactPerf = _dereq_("./ReactPerf");
  const ReactReconcileTransaction = _dereq_("./ReactReconcileTransaction");

  const getReactRootElementInContainer = _dereq_("./getReactRootElementInContainer");
  const invariant = _dereq_("./invariant");
  const setInnerHTML = _dereq_("./setInnerHTML");
 ...
 */
   ...
   */
    ...
    "mountImageIntoNode",
    (markup, container, shouldReuseMarkup) => {
      ("production" !== "development" ? invariant(
        container && (
...
*/
  ...
  const shallowEqual = _dereq_("./shallowEqual");
 ...
 */
  ...
  const ReactCurrentOwner = _dereq_("./ReactCurrentOwner");
  const ReactDescriptor = _dereq_("./ReactDescriptor");
  const ReactDescriptorValidator = _dereq_("./ReactDescriptorValidator");
  const ReactEmptyComponent = _dereq_("./ReactEmptyComponent");
  const ReactErrorUtils = _dereq_("./ReactErrorUtils");
  const ReactOwner = _dereq_("./ReactOwner");
  const ReactPerf = _dereq_("./ReactPerf");
  const ReactPropTransferer = _dereq_("./ReactPropTransferer");
  const ReactPropTypeLocations = _dereq_("./ReactPropTypeLocations");
  const ReactPropTypeLocationNames = _dereq_("./ReactPropTypeLocationNames");
  const ReactUpdates = _dereq_("./ReactUpdates");

  const instantiateReactComponent = _dereq_("./instantiateReactComponent");
  const invariant = _dereq_("./invariant");
  const keyMirror = _dereq_("./keyMirror");
  const merge = _dereq_("./merge");
  const mixInto = _dereq_("./mixInto");
  const monitorCodeUse = _dereq_("./monitorCodeUse");
  const mapObject = _dereq_("./mapObject");
  const shouldUpdateReactComponent = _dereq_("./shouldUpdateReactComponent");
  const warning = _dereq_("./warning");
 ...
 * @internal
 */
  const ReactCompositeComponentInterface = {
 ...
 */
  ...
  function validateMethodOverride(proto, name) {
    const specPolicy = ReactCompositeComponentInterface.hasOwnProperty(name) ?
  ...
  function validateLifeCycleOnReplaceState(instance) {
    const compositeLifeCycleState = instance._compositeLifeCycleState;
    ("production" !== "development" ? invariant(
    instance.isMounted() ||
 ...
 */
  function mixSpecIntoComponent(Constructor, spec) {
    ...
    for (const name in spec) {
      ...
      // 2. Overridden methods (that were mixed in).
        const isCompositeComponentMethod =
        ReactCompositeComponentInterface.hasOwnProperty(name);
        const isAlreadyDefined = proto.hasOwnProperty(name);
        const markedDontBind = property && property.__reactDontBind;
        ...
        if (shouldAutoBind) {
          ...
          if (isAlreadyDefined) {
            const specPolicy = ReactCompositeComponentInterface[name];
  ...
  function mixStaticSpecIntoComponent(Constructor, statics) {
    ...
    for (const name in statics) {
      ...
      let result = property;
      if (isInherited) {
        const existingProperty = Constructor[name];
        const existingType = typeof existingProperty;
 ...
 */
   ...
   */
    ...
    function(rootID, transaction, mountDepth) {
      ...
      this._compositeLifeCycleState = null;
      const markup = this._renderedComponent.mountComponent(
        rootID,
        transaction,
        mountDepth + 1
      );
   ...
   */
    _processContext: function(context) {
      ...
      const contextTypes = this.constructor.contextTypes;
      if (contextTypes) {
        maskedContext = {};
   ...
   */
    _processChildContext: function(currentContext) {
      const childContext = this.getChildContext && this.getChildContext();
      const displayName = this.constructor.displayName || "ReactCompositeComponent";
      if (childContext) {
        ("production" !== "development" ? invariant(
   ...
   */
    ...
    // receiving props because both of those will already be doing this.
      ...
      let nextDescriptor = this._descriptor;
      if (this._pendingDescriptor != null) {
   ...
   */
    ...
    function(transaction, prevParentDescriptor) {
      ...
      const prevDescriptor = prevComponentInstance._descriptor;
      const nextDescriptor = this._renderValidatedComponent();
      if (shouldUpdateReactComponent(prevDescriptor, nextDescriptor)) {
        prevComponentInstance.receiveComponent(nextDescriptor, transaction);
      } else {
        // These two IDs are actually the same! But nothing should rely on that.
        const thisID = this._rootNodeID;
        const prevComponentID = prevComponentInstance._rootNodeID;
        prevComponentInstance.unmountComponent();
        this._renderedComponent = instantiateReactComponent(nextDescriptor);
        const nextMarkup = this._renderedComponent.mountComponent(
          thisID,
          transaction,
          this._mountDepth + 1
        );
   ...
   */
    _bindAutoBindMethods: function() {
      for (const autoBindKey in this.__reactAutoBindMap) {
        if (!this.__reactAutoBindMap.hasOwnProperty(autoBindKey)) {
          continue;
        }
        const method = this.__reactAutoBindMap[autoBindKey];
   ...
   */
    _bindAutoBindMethod: function(method) {
      ...
      if ("production" !== "development") {
        ...
        boundMethod.bind = function(newThis) {const args=Array.prototype.slice.call(arguments,1);
  ...
  const ReactCompositeComponentBase = function() {};
 ...
 */
   ...
   */
    createClass: function(spec) {
      const Constructor = function(props, owner) {
    ...
    // Reduce time spent doing lookups by setting these on the prototype.
      for (const methodName in ReactCompositeComponentInterface) {
        if (!Constructor.prototype[methodName]) {
          Constructor.prototype[methodName] = null;
        }
      }

      const descriptorFactory = ReactDescriptor.createFactory(Constructor);
 ...
 */
  function createDOMComponentClass(omitClose, tag) {
    const Constructor = function(descriptor) {
    ...
    const ConvenienceConstructor = ReactDescriptor.createFactory(Constructor);
 ...
 */
  ...
  const AutoFocusMixin = _dereq_("./AutoFocusMixin");
  const ReactBrowserComponentMixin = _dereq_("./ReactBrowserComponentMixin");
  const ReactCompositeComponent = _dereq_("./ReactCompositeComponent");
  const ReactDOM = _dereq_("./ReactDOM");
...
// Store a reference to the <button> `ReactDOMComponent`.
  const button = ReactDOM.button;

  const mouseListenerNames = keyMirror({
    onClick: true,
    onDoubleClick: true,
    onMouseDown: true,
    onMouseMove: true,
 ...
 */
  const ReactDOMButton = ReactCompositeComponent.createClass({
 ...
 */
  ...
  const CSSPropertyOperations = _dereq_("./CSSPropertyOperations");
  const DOMProperty = _dereq_("./DOMProperty");
  const DOMPropertyOperations = _dereq_("./DOMPropertyOperations");
  const ReactBrowserComponentMixin = _dereq_("./ReactBrowserComponentMixin");
  const ReactComponent = _dereq_("./ReactComponent");
  ...
  const deleteListener = ReactBrowserEventEmitter.deleteListener;
  const listenTo = ReactBrowserEventEmitter.listenTo;
  const registrationNameModules = ReactBrowserEventEmitter.registrationNameModules;
 ...
 */
  ...
  function putListener(id, registrationName, listener, transaction) {
    const container = ReactMount.findReactContainerForID(id);
    if (container) {
 ...
 */
   ...
   */
    ...
    // bytes.
      ...
      const markupForID = DOMPropertyOperations.createMarkupForID(this._rootNodeID);
   ...
   */
    ...
    // Intentional use of != to avoid catching zero/false.
      ...
      if (innerHTML != null) {
        ...
        const childrenToUse = contentToUse != null ? null : this.props.children;
 ...
 */
  ...
  const EventConstants = _dereq_("./EventConstants");
  const LocalEventTrapMixin = _dereq_("./LocalEventTrapMixin");
  const ReactBrowserComponentMixin = _dereq_("./ReactBrowserComponentMixin");
  const ReactCompositeComponent = _dereq_("./ReactCompositeComponent");
  const ReactDOM = _dereq_("./ReactDOM");

// Store a reference to the <form> `ReactDOMComponent`.
  const form = ReactDOM.form;
 ...
 */
  const ReactDOMForm = ReactCompositeComponent.createClass({
...
/*jslint evil: true */
  ...
  const CSSPropertyOperations = _dereq_("./CSSPropertyOperations");
  const DOMChildrenOperations = _dereq_("./DOMChildrenOperations");
  const DOMPropertyOperations = _dereq_("./DOMPropertyOperations");
  const ReactMount = _dereq_("./ReactMount");
  const ReactPerf = _dereq_("./ReactPerf");
 ...
 */
   ...
   */
    ...
    "updatePropertyByID",
    (id, name, value) => {
      const node = ReactMount.getNode(id);
      ("production" !== "development" ? invariant(
 ...
 */
  const ReactDOMImg = ReactCompositeComponent.createClass({
 ...
 */
  ...
  const AutoFocusMixin = _dereq_("./AutoFocusMixin");
  const DOMPropertyOperations = _dereq_("./DOMPropertyOperations");
  const LinkedValueUtils = _dereq_("./LinkedValueUtils");
  const ReactBrowserComponentMixin = _dereq_("./ReactBrowserComponentMixin");
  const ReactCompositeComponent = _dereq_("./ReactCompositeComponent");
  const ReactDOM = _dereq_("./ReactDOM");
  const ReactMount = _dereq_("./ReactMount");
...
// Store a reference to the <input> `ReactDOMComponent`.
  const input = ReactDOM.input;

  const instancesByReactID = {};
 ...
 */
  const ReactDOMInput = ReactCompositeComponent.createClass({
    ...
    // Clone `this.props` so we don't mutate the input.
      ...
      const value = LinkedValueUtils.getValue(this);
      props.value = value != null ? value : this.state.value;

      const checked = LinkedValueUtils.getChecked(this);
    ...
    componentDidMount: function() {
      const id = ReactMount.getID(this.getDOMNode());
      instancesByReactID[id] = this;
    },

    componentWillUnmount: function() {
      const rootNode = this.getDOMNode();
      const id = ReactMount.getID(rootNode);
    ...
    _handleChange: function(event) {
      let returnValue;
      const onChange = LinkedValueUtils.getOnChange(this);
      if (onChange) {
      ...
      // `querySelectorAll` to ensure we don't miss anything.
        ...
        for (let i = 0, groupLen = group.length; i < groupLen; i++) {
          ...
          const otherID = ReactMount.getID(otherNode);
          ("production" !== "development" ? invariant(
          otherID,
          "ReactDOMInput: Mixing React and non-React radio inputs with the " +
          "same `name` is not supported."
        ) : invariant(otherID));
          const otherInstance = instancesByReactID[otherID];
          ("production" !== "development" ? invariant(
          otherInstance,
...
// Store a reference to the <option> `ReactDOMComponent`.
  const option = ReactDOM.option;

/**
 * Implements an <option> native component that warns when `selected` is set.
 */
  const ReactDOMOption = ReactCompositeComponent.createClass({
 ...
 */
  function updateOptions(component, propValue) {
    const multiple = component.props.multiple;
    const value = propValue != null ? propValue : component.state.value;
    const options = component.getDOMNode().options;
 ...
 * selected.
 */
  const ReactDOMSelect = ReactCompositeComponent.createClass({
    ...
    componentDidUpdate: function(prevProps) {
      const value = LinkedValueUtils.getValue(this);
      const prevMultiple = !!prevProps.multiple;
 ...
 */
  ...
  const getNodeForCharacterOffset = _dereq_("./getNodeForCharacterOffset");
  const getTextContentAccessor = _dereq_("./getTextContentAccessor");
 ...
 */
  function setModernOffsets(node, offsets) {
    ...
    let end = typeof offsets.end === "undefined" ?
  ...
  const useIEOffsets = ExecutionEnvironment.canUseDOM && document.selection;
...
// Store a reference to the <textarea> `ReactDOMComponent`.
  const textarea = ReactDOM.textarea;
 ...
 */
  const ReactDOMTextarea = ReactCompositeComponent.createClass({
 ...
 */
  ...
  const BeforeInputEventPlugin = _dereq_("./BeforeInputEventPlugin");
  const ChangeEventPlugin = _dereq_("./ChangeEventPlugin");
  const ClientReactRootIndex = _dereq_("./ClientReactRootIndex");
  const CompositionEventPlugin = _dereq_("./CompositionEventPlugin");
  const DefaultEventPluginOrder = _dereq_("./DefaultEventPluginOrder");
  const EnterLeaveEventPlugin = _dereq_("./EnterLeaveEventPlugin");
  const ExecutionEnvironment = _dereq_("./ExecutionEnvironment");
  const HTMLDOMPropertyConfig = _dereq_("./HTMLDOMPropertyConfig");
  const MobileSafariClickEventPlugin = _dereq_("./MobileSafariClickEventPlugin");
  const ReactBrowserComponentMixin = _dereq_("./ReactBrowserComponentMixin");
  const ReactComponentBrowserEnvironment =
  _dereq_("./ReactComponentBrowserEnvironment");
  const ReactDefaultBatchingStrategy = _dereq_("./ReactDefaultBatchingStrategy");
  const ReactDOM = _dereq_("./ReactDOM");
  const ReactDOMButton = _dereq_("./ReactDOMButton");
  const ReactDOMForm = _dereq_("./ReactDOMForm");
  const ReactDOMImg = _dereq_("./ReactDOMImg");
  const ReactDOMInput = _dereq_("./ReactDOMInput");
  const ReactDOMOption = _dereq_("./ReactDOMOption");
  const ReactDOMSelect = _dereq_("./ReactDOMSelect");
  const ReactDOMTextarea = _dereq_("./ReactDOMTextarea");
  const ReactEventListener = _dereq_("./ReactEventListener");
  const ReactInjection = _dereq_("./ReactInjection");
  const ReactInstanceHandles = _dereq_("./ReactInstanceHandles");
  const ReactMount = _dereq_("./ReactMount");
  const SelectEventPlugin = _dereq_("./SelectEventPlugin");
  const ServerReactRootIndex = _dereq_("./ServerReactRootIndex");
  const SimpleEventPlugin = _dereq_("./SimpleEventPlugin");
  const SVGDOMPropertyConfig = _dereq_("./SVGDOMPropertyConfig");

  const createFullPageComponent = _dereq_("./createFullPageComponent");
  ...
  // gets double injected.
    ...
    if ("production" !== "development") {
      ...
      if ((/[?&]react_perf\b/).test(url)) {
        const ReactDefaultPerf = _dereq_("./ReactDefaultPerf");
 ...
 */
  ...
  const DOMProperty = _dereq_("./DOMProperty");
  const ReactDefaultPerfAnalysis = _dereq_("./ReactDefaultPerfAnalysis");
  const ReactMount = _dereq_("./ReactMount");
  const ReactPerf = _dereq_("./ReactPerf");

  const performanceNow = _dereq_("./performanceNow");
  ...
  const ReactDefaultPerf = {
    ...
    measure: function(moduleName, fnName, func) {
      return function() {const args=Array.prototype.slice.call(arguments,0);
        let totalTime;
        let rv;
        let start;
        ...
        moduleName === "ReactComponentBrowserEnvironment") {
          ...
          if (fnName === "mountImageIntoNode") {
            const mountID = ReactMount.getID(args[1]);
            ReactDefaultPerf._recordWrite(mountID, fnName, totalTime, args[0]);
          } else if (fnName === "dangerouslyProcessChildrenUpdates") {
          // special format
            args[0].forEach(update => {
            const writeArgs = {};
        ...
        fnName === "_renderValidatedComponent")) {
          ...
          const rootNodeID = fnName === "mountComponent" ?
          args[0] :
          this._rootNodeID;
          ...
          const entry = ReactDefaultPerf._allMeasurements[
          ReactDefaultPerf._allMeasurements.length - 1
        ];
...
// Don't try to save users less than 1.2ms (a number I made up)
  ...
  function getDOMSummary(measurements) {
    ...
    for (let i = 0; i < measurements.length; i++) {
      ...
      for (id in measurement.writes) {
        measurement.writes[id].forEach(write => {
          items.push({
            id: id,
  ...
  function getExclusiveSummary(measurements) {
    ...
    for (let i = 0; i < measurements.length; i++) {
      const measurement = measurements[i];
      const allIDs = merge(measurement.exclusive, measurement.inclusive);
  ...
  // the amount of time it took to render the entire subtree.
    ...
    const dirtyLeafIDs = Object.keys(measurement.writes);
    const allIDs = merge(measurement.exclusive, measurement.inclusive);
    ...
    // a DOM op is in its subtree.
      for (let i = 0; i < dirtyLeafIDs.length; i++) {
 ...
 */
  ...
  const ownerHasMonitoredObjectMap = {};

  const loggedTypeFailures = {};

  const NUMERIC_PROPERTY_REGEX = /^\d+$/;
 ...
 */
  function getCurrentOwnerDisplayName() {
    const current = ReactCurrentOwner.current;
 ...
 */
  function warnAndMonitorForKeyUse(warningID, message, component, parentType) {
    const ownerName = getCurrentOwnerDisplayName();
    const parentName = parentType.displayName;

    const useName = ownerName || parentName;
    const memoizer = ownerHasKeyUseWarning[warningID];
 ...
 */
  function monitorUseOfObjectMap() {
    const currentName = getCurrentOwnerDisplayName() || "";
 ...
 */
  ...
  const ReactDescriptorValidator = {
   ...
   */
    createFactory: function(factory, propTypes, contextTypes) {
      const validatedFactory = function(props, children) {
        const descriptor = factory.apply(this, arguments);

        for (let i = 1; i < arguments.length; i++) {
          validateChildKeys(arguments[i], descriptor.type);
        }

        const name = descriptor.type.displayName;
        if (propTypes) {
          checkPropTypes(
          name,
          propTypes,
...
// `null` (in reality a placeholder such as `noscript`)
  const nullComponentIdsRegistry = {};
 ...
 */
  ...
  const EventListener = _dereq_("./EventListener");
  const ExecutionEnvironment = _dereq_("./ExecutionEnvironment");
  const PooledClass = _dereq_("./PooledClass");
  const ReactInstanceHandles = _dereq_("./ReactInstanceHandles");
  const ReactMount = _dereq_("./ReactMount");
  ...
  const getEventTarget = _dereq_("./getEventTarget");
 ...
 */
  ...
  // mutation observer to listen for all DOM changes.
    const nodeID = ReactMount.getID(node);
    const rootID = ReactInstanceHandles.getReactRootIDFromNodeID(nodeID);
    const container = ReactMount.findReactContainerForID(rootID);
    const parent = ReactMount.getFirstReactDOM(container);
    return parent;
  }
...
// Used to store ancestor hierarchy in top level callback
  ...
  function handleTopLevelImpl(bookKeeping) {
    let topLevelTarget = ReactMount.getFirstReactDOM(
  ...
  // inconsistencies with ReactMount's node cache. See #1105.
    let ancestor = topLevelTarget;
    while (ancestor) {
      bookKeeping.ancestors.push(ancestor);
      ancestor = findParent(ancestor);
    }

    for (let i = 0, l = bookKeeping.ancestors.length; i < l; i++) {
      topLevelTarget = bookKeeping.ancestors[i];
      const topLevelTargetID = ReactMount.getID(topLevelTarget) || "";
 ...
 */
  ...
  const ReactDOMSelection = _dereq_("./ReactDOMSelection");

  const containsNode = _dereq_("./containsNode");
  const focusNode = _dereq_("./focusNode");
  const getActiveElement = _dereq_("./getActiveElement");
...
// Used to store breadth-first search state in findComponentRoot.
  const findComponentRootReusableArray = [];
 ...
 */
  function setID(node, id) {
    const oldID = internalGetID(node);
    if (oldID !== id) {
 ...
 */
  ...
  let deepestNodeSoFar = null;
  function findDeepestCachedAncestorImpl(ancestorID) {
    const ancestor = nodeCache[ancestorID];
 ...
 */
  function findDeepestCachedAncestor(targetID) {
    ...
    const foundNode = deepestNodeSoFar;
    deepestNodeSoFar = null;
    return foundNode;
  }
 ...
 */
   ...
   */
    ...
    (
      ...
      const componentInstance = instantiateReactComponent(nextComponent);
      const reactRootID = ReactMount._registerComponent(
        componentInstance,
        container
      );
   ...
   */
    renderComponent: function(nextDescriptor, container, callback) {
      ...
      const prevComponent = instancesByReactRootID[getReactRootID(container)];

      if (prevComponent) {
        const prevDescriptor = prevComponent._descriptor;
        if (shouldUpdateReactComponent(prevDescriptor, nextDescriptor)) {
   ...
   */
    ...
    // render but we still don't expect to be in a render call here.)
      ...
      const reactRootID = getReactRootID(container);
      const component = instancesByReactRootID[reactRootID];
      if (!component) {
        return false;
      }
   ...
   */
    findReactContainerForID: function(id) {
      ...
      const container = containersByReactRootID[reactRootID];

      if ("production" !== "development") {
        const rootElement = rootElementsByReactRootID[reactRootID];
   ...
   */
    findReactNodeByID: function(id) {
      const reactRoot = ReactMount.findReactContainerForID(id);
   ...
   */
    isRenderedByReact: function(node) {
      ...
      const id = ReactMount.getID(node);
   ...
   */
    findComponentRoot: function(ancestorNode, targetID) {
      const firstChildren = findComponentRootReusableArray;
      let childIndex = 0;

      const deepestAncestor = findDeepestCachedAncestor(targetID) || ancestorNode;
      ...
      while (childIndex < firstChildren.length) {
        ...
        while (child) {
          const childID = ReactMount.getID(child);
          if (childID) {
 ...
 */
  ...
  const flattenChildren = _dereq_("./flattenChildren");
  const instantiateReactComponent = _dereq_("./instantiateReactComponent");
  const shouldUpdateReactComponent = _dereq_("./shouldUpdateReactComponent");
   ...
   */
     ...
     */
      mountChildren: function(nestedChildren, transaction) {
        const children = flattenChildren(nestedChildren);
        const mountImages = [];
        let index = 0;
     ...
     */
      _updateChildren: function(nextNestedChildren, transaction) {
        const nextChildren = flattenChildren(nextNestedChildren);
        const prevChildren = this._renderedChildren;
      ...
      // `lastIndex` will be the last index visited in `prevChildren`.
        ...
        for (name in nextChildren) {
          ...
          const prevDescriptor = prevChild && prevChild._descriptor;
          const nextDescriptor = nextChildren[name];
          if (shouldUpdateReactComponent(prevDescriptor, nextDescriptor)) {
          ...
          // The child must be instantiated before it's mounted.
            const nextChildInstance = instantiateReactComponent(nextDescriptor);
     ...
     */
      ...
      // Inlined for performance, see `ReactInstanceHandles.createReactID`.
        const rootID = this._rootNodeID + name;
        const mountImage = child.mountComponent(
        rootID,
        transaction,
        this._mountDepth + 1
      );
 ...
 */
  ...
  const emptyObject = _dereq_("./emptyObject");
  const invariant = _dereq_("./invariant");
   ...
   */
     ...
     */
      attachRef: function(ref, component) {
        ...
        const refs = this.refs === emptyObject ? (this.refs = {}) : this.refs;
        refs[ref] = component;
      },
  ...
  function createArrayOfTypeChecker(typeChecker) {
    function validate(props, propName, componentName, location) {
      ...
      for (let i = 0; i < propValue.length; i++) {
        const error = typeChecker(propValue, i, componentName, location);
        if (error instanceof Error) {
          return error;
        }
      }
    ...
    function validate(props, propName, componentName, location) {
      ...
      for (const key in propValue) {
        if (propValue.hasOwnProperty(key)) {
          const error = typeChecker(propValue, key, componentName, location);
          if (error instanceof Error) {
            return error;
          }
        }
  ...
  function renderComponentToString(component) {
    ...
    try {
      ...
      return transaction.perform(() => {
        ...
        const markup = componentInstance.mountComponent(id, transaction, 0);
 ...
 */
   ...
   */
    scryRenderedDOMComponentsWithClass: function(root, className) {
      return ReactTestUtils.findAllInRenderedTree(root, inst => {
        const instClassName = inst.props.className;
   ...
   */
    mockComponent: function(module, mockTagName) {
      const ConvenienceConstructor = React.createClass({
        render: function() {
          const mockTagName = mockTagName || module.mockTagName || "div";
 ...
 * @internal
 */
  const ReactTextComponent = function(descriptor) {
    this.construct(descriptor);
  };
   ...
   */
    mountComponent: function(rootID, transaction, mountDepth) {
      ...
      const escapedText = escapeTextForBrowser(this.props);
 ...
 */
  ...
  const React = _dereq_("./React");
  const ReactTransitionChildMapping = _dereq_("./ReactTransitionChildMapping");

  const cloneWithProps = _dereq_("./cloneWithProps");
  const emptyFunction = _dereq_("./emptyFunction");
  const merge = _dereq_("./merge");
  ...
  const ReactTransitionGroup = React.createClass({
    ...
    _handleDoneLeaving: function(key) {
      ...
      // This entered again before it fully left. Add it again.
        ...
        const newChildren = merge(this.state.children);
 ...
 */
  ...
  const flushBatchedUpdates = ReactPerf.measure(
  "ReactUpdates",
  "flushBatchedUpdates",
  () => {
 ...
 */
  ...
  const LinkedStateMixin = _dereq_("./LinkedStateMixin");
  const React = _dereq_("./React");
  const ReactComponentWithPureRenderMixin =
  _dereq_("./ReactComponentWithPureRenderMixin");
  const ReactCSSTransitionGroup = _dereq_("./ReactCSSTransitionGroup");
  const ReactTransitionGroup = _dereq_("./ReactTransitionGroup");

  const cx = _dereq_("./cx");
  const cloneWithProps = _dereq_("./cloneWithProps");
  const update = _dereq_("./update");
 ...
 */
  const GLOBAL_MOUNT_POINT_MAX = Math.pow(2, 53);

  const ServerReactRootIndex = {
    createReactRootIndex: function() {
 ...
 */
  ...
  const EventPluginUtils = _dereq_("./EventPluginUtils");
  const EventPropagators = _dereq_("./EventPropagators");
  const SyntheticClipboardEvent = _dereq_("./SyntheticClipboardEvent");
  const SyntheticEvent = _dereq_("./SyntheticEvent");
  const SyntheticFocusEvent = _dereq_("./SyntheticFocusEvent");
  const SyntheticKeyboardEvent = _dereq_("./SyntheticKeyboardEvent");
  const SyntheticMouseEvent = _dereq_("./SyntheticMouseEvent");
  const SyntheticDragEvent = _dereq_("./SyntheticDragEvent");
  const SyntheticTouchEvent = _dereq_("./SyntheticTouchEvent");
  const SyntheticUIEvent = _dereq_("./SyntheticUIEvent");
  const SyntheticWheelEvent = _dereq_("./SyntheticWheelEvent");
  ...
  for (const topLevelType in topLevelEventsToDispatchConfig) {
   ...
   */
    executeDispatch: function(event, listener, domID) {
      const returnValue = EventPluginUtils.executeDispatch(event, listener, domID);
 ...
 */
  ...
  const getEventKey = _dereq_("./getEventKey");
  const getEventModifierState = _dereq_("./getEventModifierState");
 ...
 */
  ...
  const isTextNode = _dereq_("./isTextNode");
 ...
 */
  function createFullPageComponent(componentClass) {
    const FullPageComponent = ReactCompositeComponent.createClass({
...
/*jslint evil: true, sub: true */
  ...
  const createArrayFrom = _dereq_("./createArrayFrom");
  const getMarkupWrap = _dereq_("./getMarkupWrap");
  const invariant = _dereq_("./invariant");
 ...
 */
  function createNodesFromMarkup(markup, handleScript) {
    ...
    const nodes = createArrayFrom(node.childNodes);
 ...
 */
  function cx(classNames) {
    if (typeof classNames == "object") {
      return Object.keys(classNames).filter(className => {
 ...
 */
  ...
  // We found a component instance.
    ...
    const keyUnique = !result.hasOwnProperty(name);
    ("production" !== "development" ? warning(
    keyUnique,
 ...
 */
  ...
  // Browser does not implement `key`, polyfill as much of it as we can.
    ...
    // perfect replacement.
      const charCode = "charCode" in nativeEvent ?
 ...
 */
  ...
  const hyphenate = _dereq_("./hyphenate");
 ...
 */
  ...
  let performance = _dereq_("./performance");
 ...
 * @internal
 */
  let setInnerHTML = function(node, html) {
    node.innerHTML = html;
  };
  ...
  function(children, nameSoFar, indexSoFar, callback, traverseContext) {
    ...
    if (Array.isArray(children)) {
      for (let i = 0; i < children.length; i++) {
        ...
        const nextIndex = indexSoFar + subtreeCount;
      ...
      if (children == null || type === "boolean") {
        ...
        if (type === "object") {
          ...
          const normalizedText = ReactTextComponent(children);
          callback(traverseContext, normalizedText, storageName, indexSoFar);
          subtreeCount += 1;
        } else if (type === "number") {
          const normalizedNumber = ReactTextComponent("" + children);
 ...
 */
  ...
  if ("production" !== "development") {
    warning = function(condition, format) {const args=Array.prototype.slice.call(arguments,2);
      if (format === undefined) {
        throw new Error(
      ...
      if (!condition) {
        let argIndex = 0;
        console.warn("Warning: " + format.replace(/%s/g, () => {return args[argIndex++];}));
      }
    };
  }
