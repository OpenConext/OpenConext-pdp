...
(function(mod) {
  ...
  let gecko = /gecko\/\d/i.test(navigator.userAgent);
  let ie_upto10 = /MSIE \d/.test(navigator.userAgent);
  let ie_11up = /Trident\/(?:[7-9]|\d{2,})\..*rv:(\d+)/.exec(navigator.userAgent);
  ...
  let qtwebkit = webkit && /Qt\/\d+\.\d+/.test(navigator.userAgent);
  let chrome = /Chrome\//.test(navigator.userAgent);
  let presto = /Opera\//.test(navigator.userAgent);
  let safari = /Apple Computer/.test(navigator.vendor);
  let mac_geMountainLion = /Mac OS X 1\d\D([8-9]|\d\d)\D/.test(navigator.userAgent);
  let phantom = /PhantomJS/.test(navigator.userAgent);

  let ios = /AppleWebKit/.test(navigator.userAgent) && /Mobile\/\w+/.test(navigator.userAgent);
  // This is woefully incomplete. Suggestions for alternative methods welcome.
  let mobile = ios || /Android|webOS|BlackBerry|Opera Mini|Opera Mobi|IEMobile/i.test(navigator.userAgent);
  let mac = ios || /Mac/.test(navigator.platform);
  let windows = /win/i.test(navigator.platform);
  ...
  function setDocumentHeight(cm, measure) {
    ...
    let total = measure.docHeight + cm.display.barHeight;
  ...
  let Pos = CodeMirror.Pos = function(line, ch) {
  ...
  let cmp = CodeMirror.cmpPos = function(a, b) { return a.line - b.line || a.ch - b.ch; };
  ...
  function handlePaste(e, cm) {
    let pasted = e.clipboardData && e.clipboardData.getData("text/plain");
    if (pasted) {
      e.preventDefault();
  ...
  function hiddenTextarea() {
    let te = elt("textarea", null, null, "position: absolute; padding: 0; width: 1px; height: 1em; outline: none");
  ...
  TextareaInput.prototype = copyObj({
    ...
    onContextMenu: function(e) {
      ...
      if (webkit) let oldScrollY = window.scrollY; // Work around Chrome issue (#2712)
      display.input.focus();
  ...
  ContentEditableInput.prototype = copyObj({
    init: function(display) {
      ...
      on(div, "paste", (e) => { handlePaste(e, cm); });
  ...
  function skipAtomicInSelection(doc, sel, bias, mayClear) {
    ...
    for (let i = 0; i < sel.ranges.length; i++) {
      let range = sel.ranges[i];
      let newAnchor = skipAtomic(doc, range.anchor, bias, mayClear);
      let newHead = skipAtomic(doc, range.head, bias, mayClear);
  ...
  function skipAtomic(doc, pos, bias, mayClear) {
    ...
    doc.cantEdit = false;
    search: for (;;) {
      let line = getLine(doc, curPos.line);
      if (line.markedSpans) {
  ...
  function coordsCharInner(cm, lineObj, lineNo, x, y) {
    ...
    for (;;) {
      if (bidi ? to == from || to == moveVisually(lineObj, from, 1) : to - from <= 1) {
        ...
        let pos = PosWithInfo(lineNo, ch, ch == from ? fromOutside : toOutside,
  ...
  function endOperation(cm) {
    let op = cm.curOp, group = op.ownsGroup;
    if (!group) return;
  ...
  function dragDropChanged(cm, value, old) {
    let wasOn = old && old != CodeMirror.Init;
    if (!value != !wasOn) {
      ...
      try {
        ...
        if (text) {
          if (cm.state.draggingText && !(mac ? e.altKey : e.ctrlKey))
            let selected = cm.listSelections();
  ...
  function handleCharBinding(cm, e, ch) {
    return dispatchKey(cm, "'" + ch + "'", e,
                       (b) => { return doHandleBinding(cm, b, true); });
  }
  ...
  let changeEnd = CodeMirror.changeEnd = function(change) {
  ...
  function findPosH(doc, pos, dir, unit, visually) {
    ...
    let result = skipAtomic(doc, Pos(line, ch), origDir, true);
  ...
  let defaults = CodeMirror.defaults = {};
  ...
  let Init = CodeMirror.Init = { toString: function(){return "CodeMirror.Init";} };
  ...
  option("keyMap", "default", (cm, val, old) => {
    ...
    let prev = old != CodeMirror.Init && getKeyMap(old);
  ...
  let modes = CodeMirror.modes = {}, mimeModes = CodeMirror.mimeModes = {};
  ...
  let modeExtensions = CodeMirror.modeExtensions = {};
  ...
  let copyState = CodeMirror.copyState = function(mode, state) {
  ...
  let startState = CodeMirror.startState = function(mode, a1, a2) {
  ...
  let commands = CodeMirror.commands = {
    ...
    goLineStart: function(cm) {
      cm.extendSelectionsBy((range) => { return lineStart(cm, range.head.line); },
                            { origin: "+move", bias: 1 });
    },
    ...
    goLineEnd: function(cm) {
      cm.extendSelectionsBy((range) => { return lineEnd(cm, range.head.line); },
                            { origin: "+move", bias: -1 });
    },
  ...
  let lookupKey = CodeMirror.lookupKey = function(key, map, handle, context) {
    map = getKeyMap(map);
  ...
  let isModifierKey = CodeMirror.isModifierKey = function(value) {
    let name = typeof value == "string" ? value : keyNames[value.keyCode];
    return name == "Ctrl" || name == "Alt" || name == "Shift" || name == "Mod";
  };

  // Look up the name of a key as indicated by an event object.
  let keyName = CodeMirror.keyName = function(event, noShift) {
  ...
  let StringStream = CodeMirror.StringStream = function(string, tabSize) {
  ...
  let TextMarker = CodeMirror.TextMarker = function(doc, type) {
    this.lines = [];
    this.type = type;
    this.doc = doc;
  ...
  // markers.
  let SharedTextMarker = CodeMirror.SharedTextMarker = function(markers, primary) {
  ...
  let LineWidget = CodeMirror.LineWidget = function(doc, node, options) {
  ...
  let Line = CodeMirror.Line = function(text, markedSpans, estimateHeight) {
    this.text = text;
  ...
  function callBlankLine(mode, state) {
    ...
    let inner = CodeMirror.innerMode(mode, state);
  ...
  function runMode(cm, text, mode, state, f, lineClasses, forceToEnd) {
    ...
    while (curStart < stream.pos) {
      ...
      let pos = Math.min(stream.pos, curStart + 50000);
      f(pos, curStyle);
      curStart = pos;
    }
  }
  ...
  function buildToken(builder, text, style, startStyle, endStyle, title, css) {
    if (!text) return;
    let displayText = builder.splitSpaces ? text.replace(/ {3,}/g, splitSpaces) : text;
  ...
  let nextDocId = 0;
  let Doc = CodeMirror.Doc = function(text, mode, firstLine, lineSep) {
  ...
  let e_preventDefault = CodeMirror.e_preventDefault = function(e) {
    if (e.preventDefault) e.preventDefault();
    else e.returnValue = false;
  };
  let e_stopPropagation = CodeMirror.e_stopPropagation = function(e) {
  ...
  let e_stop = CodeMirror.e_stop = function(e) {e_preventDefault(e); e_stopPropagation(e);};
  ...
  let on = CodeMirror.on = function(emitter, type, f) {
  ...
  let off = CodeMirror.off = function(emitter, type, f) {
  ...
  let signal = CodeMirror.signal = function(emitter, type /*, values...*/) {
    let arr = emitter._handlers && emitter._handlers[type];
    if (!arr) return;
  ...
  // handling this'.
  let Pass = CodeMirror.Pass = { toString: function(){return "CodeMirror.Pass";} };
  ...
  let countColumn = CodeMirror.countColumn = function(string, end, tabSize, startIndex, startValue) {
    if (end == null) {
      end = string.search(/[^\s\u00a0]/);
  ...
  // a particular column.
  let findColumn = CodeMirror.findColumn = function(string, goal, tabSize) {
    for (let pos = 0, col = 0;;) {
  ...
  let isWordCharBasic = CodeMirror.isWordChar = function(ch) {
  ...
  let contains = CodeMirror.contains = function(parent, child) {
  ...
  let rmClass = CodeMirror.rmClass = function(node, cls) {
  ...
  let addClass = CodeMirror.addClass = function(node, cls) {
    let current = node.className;
  ...
  let splitLinesAuto = CodeMirror.splitLines = "\n\nb".split(/\n/).length != 3 ? function(string) {
  ...
  let keyNames = CodeMirror.keyNames = {
